...
=====
Summary of LRP options with general estimation, method of calculation, and criteria (see next page).

```{r}

knitr::include_graphics("images/Presentation2/SAR_table1.PNG")
...
```{r}
plot(sr_noCollapse$biomass_tm2, sr_noCollapse$R, xlim=xlmts, ylim=ylmts, col="white", ylab="Recruits (billions)", xlab = "Biomass (ktonnes)")
...
### All
```{r}
plot(sr$biomass_tm2, sr$R, pch=16, col='steelblue', ylab = 'Recruits (billions; t)', xlab = 'biomass (kt; t-2)')
...
#df_keep_rows <- keep_rows("N2", 25)
#tab1 <- tabParm(histLRP, df_keep_rows)
datatable(histB01, rownames = F, class = 'cell-border stripe', height = 50,  options = list(searching = F, pageLength = 30, autoWidth = TRUE, scrollY = F))
```
...
knitr::include_graphics("images/Presentation2/SAR_table2.PNG")
