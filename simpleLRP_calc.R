...
### R1 ----
# biomass[t-2] v R[t]
### The Ricker curve doesn't look bad with this approach
### But, I get negative values for the a and b parameter which results in a negative Bmsy - nonsensical result assuming i've done it right.
...
# lines(pR~xr, col="red") # Ricker Curve
# abline(h=max(pR)) # max of the Ricker Curve
# abline(h=0.5*(max(pR)))
...
# ###### THESE ARE FOR SEMELPAROUS SPECIES
...
# ### BH1 ----
# #### This uses age2PerMat which is NA
# 
# lm.sr2 <- lm(I(1/(R)) ~ I(1/biomass_tm2), data = sr)
# summary(lm.sr2)
...
# srBH <- sr2 
# # plot biomass v R/S and overlay predicted values
# plot(srBH$biomass_tm2, log(srBH$age2/srBH$biomass_tm2))
# plot(srBH$biomass_tm2, srBH$age2/srBH$biomass_tm2)
# lines(srBH$biomass_tm2, srBH$predict)
# 
# plot(srBH$biomass_tm2, srBH$age2)
...
# tmp <- nls(age2 ~ biomass_tm2*alpha/(1 + biomass_tm2/k), data = srBH, start = list(alpha = 5, k = 50))
# 
# 
# # Trying to get spawners v recruits
# plot(srBH$biomass, srBH$age2)
# 
# 
# plot(srBH$biomass, log(srBH$age2/srBH$biomass))
...
# different formulation of BH
# svBH <- srStarts(age2~biomass_tm2,data=na.omit(sr),type="BevertonHolt",2)
# bhr <- srFuns("BevertonHolt" ,2)
# srFuns("BevertonHolt",2) # this is last column in HW 1992 Table 7.2 bc Rp = b
# bhr(S=1350, a = svBH$a, svBH$Rp)
# srBH1 <- nls(R ~ bhr(biomass_tm2, a, b), data = na.omit(sr1), start=svBH)
# 
# 
